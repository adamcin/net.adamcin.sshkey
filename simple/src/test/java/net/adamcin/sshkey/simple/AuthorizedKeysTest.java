...
public class AuthorizedKeysTest {
    ...
    public void compareAuthorizedKeyToKeyPair(String parentName, String keyName, KeyFormat format) {
        ...
        try {
            ...
            PublicPair publicPair = AuthorizedKeys.readPublicPair(Base64.fromBase64String(key.getEncodedKey(), false));
            ...
            Identity jceIdentity = new JCEIdentity(format, keyPair);
            Identity akIdentity = AuthorizedKeys.createPublicIdentity(publicPair);

            assertEquals(id + "fingerprints should match", jceIdentity.getFingerprint(), akIdentity.getFingerprint());

            SimpleIdentityProvider signingAndVerifying = new SimpleIdentityProvider(Arrays.asList(jceIdentity));
            SimpleIdentityProvider verifying = new SimpleIdentityProvider(Arrays.asList(akIdentity));
            ...
            final String fingerprint = jceIdentity.getFingerprint();
